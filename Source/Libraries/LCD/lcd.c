...
static void LCD_WR_DATA(uint16_t LCD_Data)
{
	...
	GPIO_ResetBits(GPIOC, GPIO_Pin_6);  // LCD_CS(0);
	GPIO_ResetBits(GPIOD, GPIO_Pin_14); // LCD WR(0);
...
void LCD_ShowCharSizedString(uint16_t x, uint16_t y, const uint8_t *p, 	uint8_t size,
						uint16_t PenColor, uint16_t BackColor)
{
...
void LCD_ShowSizedString(u16 x0, u16 y0, u8 *pcStr, u8 size, u16 PenColor, u16 BackColor)
{
	while (*pcStr != '\0')
	{
		if (*pcStr > 0x80)
		{
			LCD_ShowHzSizedString(x0, y0, pcStr, size, PenColor, BackColor);
			pcStr += 2;
			x0 +=size;
		}
		else
		{
			LCD_ShowCharSizedString(x0, y0, pcStr, size, PenColor, BackColor);
			pcStr += 1;
			x0 += size/2;
		}
	}
...
static void WriteOneSizedHz(u16 x0, u16 y0, u8 *pucMsk, u16 size, u16 PenColor, u16 BackColor)
{
	u16 i, j;
	u16 mod[16];
	u16 *pusMsk;
...
void LCD_ShowHzSizedString(u16 x0, u16 y0, u8 *pcStr, u8 size, u16 PenColor, u16 BackColor)
{
#define MAX_HZ_POSX 224
#define MAX_HZ_POSY 304
	u16 usIndex;
	...
	usIndex = findHzIndex(pcStr);
	WriteOneSizedHz(x0, y0, (u8 *)&(ptGb16[usIndex].Msk[0]), (u16)size, PenColor, BackColor);
}
